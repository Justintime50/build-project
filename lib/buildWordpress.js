...
function buildWordpressProject(filepath) {
    ...
    try {
        ...
        exec(`cd ${filepath} && curl -sLJO https://github.com/WordPress/WordPress/archive/${wordpressVersion}.zip && unzip -q WordPress-${wordpressVersion}.zip && mv Wordpress-${wordpressVersion} src && rm WordPress-${wordpressVersion}.zip`, (err) => {
            if (err) {
                console.log(err)
                return err
            }
